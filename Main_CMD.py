...
def adaptive_good_features(gray, min_features=50, max_features=500, wind_factor=1.0):
    ...
    # Extract scalar values from (1,1) arrays
    std_scalar = std_val[0,0]
    quality_level = max(0.01, 0.1 * (1 - std_scalar / 50))
    quality_level = float(quality_level)  # Ensure it's a float
    ...
    # IMPORTANT: Use named arguments after minDistance to avoid mask confusion
    pts = cv2.goodFeaturesToTrack(
        ...
        # mask is omitted → use named args!
    )
    return pts
